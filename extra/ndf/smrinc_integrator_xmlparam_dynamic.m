function integrator = smrinc_integrator_xmlparam_dynamic(config, integrator)
    ...
    if(isnan(integrator.param.phi))
        warning('chk:param', '[ndf_smrinc] - phi parameter not defined in xml. Using default value');
        integrator.param.chi = DEFAULT_PHI;
    end
    
    if(isnan(integrator.param.chi))
        warning('chk:param', '[ndf_smrinc] - chi parameter not defined in xml. Using default value');
        integrator.param.chi = DEFAULT_CHI;
    end
    
    if(isnan(integrator.param.inc))
        warning('chk:param', '[ndf_smrinc] - inc parameter not defined in xml. Using default value');
        integrator.param.inc = DEFAULT_INC;
    end
    
    if(isnan(integrator.param.nrp))
        warning('chk:param', '[ndf_smrinc] - nrp parameter not defined in xml. Using default value');
