...
try 
	...
	if(cl_connect(loop.cl) == false)
		disp('[ndf_smr_mobile] Cannot connect to CNBI Loop, killing matlab');
		exit;
	end	
	...
	if(loop.cfg.config == 0)
		disp('[ndf_smr_mobile] Cannot retrieve loop configuration, killing matlab');
		exit;
	end
	...
	if(isempty(loop.cfg.ndf.pipe))
		disp('[ndf_smr_mobile] NDF configuration failed, killing matlab:');
	...
	disp('[ndf_smr_mobile] Receiving ACK...');
    ...
    if isempty(integrator.type)
        disp('[ndf_smr_mobile] Cannot retrieve the integrator type from nameserver')
        disp('[ndf_smr_mobile] Killing MATLAB')
        exit;
    end
    ...
    if isempty(control.type)
        disp('[ndf_smr_mobile] Cannot retrieve the control type from nameserver')
        disp('[ndf_smr_mobile] Killing MATLAB')
        exit;
    end
    switch(control.type)
        ...
        otherwise
            disp(['[ndf_smr_mobile] - Unknown control type: ' control.type '. Exit']);
            exit;
    end
    disp(['[ndf_smr_mobile] - Control type set as ' control.type])
	...
	if(user.bci.analysis.settings.eeg.fs ~= ndf.conf.sf)
		disp('[ndf_smr_mobile] Error: NDF sample rate differs from classifier settings');
        disp('[ndf_smr_mobile] Killing Matlab...');
        exit;
	end
	...
	disp(['[ndf_smr_mobile] NDF type id: ' num2str(ndf.conf.id)]);
	...
	%   running too slow
	disp('[ndf_smr_mobile] Receiving NDF frames...');
	...
	while(true)
		...
		if(ndf.size == 0)
			disp('[ndf_smr_mobile] Broken pipe');
			break;
		end
            ...
            case 'ema'
                ...
                if(isempty(useralpha) == false)
                    if(str2double(useralpha) ~= double(integrator.param.alpha))
                        ...
                        disp(['[ndf_smr_mobile] - DEBUG: Changed alpha parameter for ema to: ' useralpha]);
                    end
                end
            ...
            case 'dynamic'
                ...
                if(isempty(userphi) == false)
                    if(str2double(userphi) ~= double(integrator.param.phi))
                        ...
                        disp(['[ndf_smr_mobile] - DEBUG: Changed phi parameter for dynamic to: ' userphi]);
                    end
                end
        ...
        if(tid_isattached(loop.iD) == true)
            while(tid_getmessage(loop.iD, loop.sDi) == true)
                ...
                if(id_event == 781)
                    integrator.nprobs = 0.5;
					printf('[ndf_smr_mobile] Resetting NDF=%d/iD=%d\n', ndf.frame.index, idmessage_getbidx(loop.mDi));
                end
            end
        else
        ...
        if(tic_isattached(loop.iC) == true)
			for t = 1:user.nTasks
                ...
                if length(taskloc) ~= 1
                    disp('[ndf_smr_mobile] Task not present in classifier')
                    disp('[ndf_smr_mobile] Killing MATLAB')
                    exit;
                end
        ...
        if(loop.jump.isjumping)
			disp(['[ndf_smr_mobile] Error: running slow ' num2str(ndf_toc(loop.uptime)/1000/60)]);
        end
        
	end
catch exception
	ndf_printexception(exception);
	disp('[ndf_smr_mobile] Going down');
	loop.exit = true;
end
...
disp(['[ndf_smr_mobile] Loop uptime: ' ...
