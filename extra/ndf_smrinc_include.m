function ndf_smrinc_include()

try
	if(isempty(getenv('CNBITKMAT_ROOT')))
		disp('[ndf_smrinc_include] $CNBITKMAT_ROOT not found, using default');
		setenv('CNBITKMAT_ROOT', '/usr/share/cnbiloop/cnbitkmat/');
	end
	if(isempty(getenv('EEGC3_ROOT')))
		disp('[ndf_smrinc_include] $EEGC3_ROOT not found, using default');
	...
	if(isempty(which('mtpath_include')))
		disp('[ndf_smrinc_include] mtpath not installed, killing Matlab');
		exit;
	end
	...
	disp(['[ndf_smrinc_include] Exception: ' exception.message ]);
	disp(exception);
	disp(exception.stack);
	disp('[ndf_smrinc_include] Killing Matlab...');
	exit;
end
